...
check('all three modules are wired into MODULES', () => {
	assert(/const MODULES = \[TextureLayersModule, OneSidedStretchModule, LayeredLockAlphaModule\];/.test(source),
...
check('each module is still there, by the settings it owns', () => {
	...
	for (const id of ['embodygames_persist_texture_layers', 'one_sided_stretch_step', 'lla_enabled']) {
...
check('the module banners are intact', () => {
	...
	for (const banner of ['1/3  TEXTURE LAYERS', '2/3  ONE-SIDED STRETCH', '3/3  LAYERED LOCK ALPHA']) {
